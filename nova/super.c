...
module_param(disable_gc, int, 0444);
MODULE_PARM_DESC(disable_gc, "Disable garbage collection");
...
module_param(fragment, int, 0444);
MODULE_PARM_DESC(fragment, "Control debugging output");

module_param(enhance, int, 0444);
MODULE_PARM_DESC(enhance, "Control debugging output");
...
static int nova_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	nova_dbg("measure timing %d, metadata checksum %d, wprotect %d, disable_gc %d, data checksum %d, data parity %d, DRAM checksum %d, fragment %d, enhance %d\n",
		measure_timing, metadata_csum, 
		wprotect,	 disable_gc, data_csum,
...
static void nova_put_super(struct super_block *sb)
{
	...
	INIT_TIMING(umount_time);
	NOVA_START_TIMING(umount_t, umount_time);
	nova_print_curr_epoch_id(sb);
...
//	nova_print_free_lists(sb);
	...
	NOVA_END_TIMING(umount_t, umount_time);
