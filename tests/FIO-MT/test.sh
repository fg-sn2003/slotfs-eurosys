            ...
            for job in "${NUM_JOBS[@]}"; do
                ...
                if [[ "${file_system}" == "SLOTFS" ]]; then
                    ...
                    if (( job == 1 )); then
                        ...
                        BW=$(LD_PRELOAD=$SLOTFS_DIR/build/libslot.so fio -directory="\\/" -fallocate=none -direct=0 -iodepth 1 -thread -numjobs="$job" --overwrite=1 -rw=write -ioengine=sync -bs="4K" -size="$fsize"M -name=test | grep WRITE: | awk '{print $2}' | sed 's/bw=//g' | "$TOOLS_PATH"/converter/to_MiB_s)
                    fi
                ...
                if [[ "${file_system}" == "SLOTFS" ]]; then
                    ...
                    export SLOTFS_MKFS=1
                    if (( job == 1 )); then
                        BW=$(LD_PRELOAD=$SLOTFS_DIR/build/libslot.so fio -filename="\\test" -fallocate=none -direct=0 -iodepth 1 -rw=read -ioengine=sync -bs="4K" -size="$fsize"M -name=test | grep READ: | awk '{print $2}' | sed 's/bw=//g' | "$TOOLS_PATH"/converter/to_MiB_s)
                    else
                        BW=$(LD_PRELOAD=$SLOTFS_DIR/build/libslot.so fio -directory="dir" -fallocate=none -direct=0 -iodepth 1 -thread -numjobs="$job" -rw=read -ioengine=sync -bs="4K" -size="$fsize"M -name=test | grep READ: | awk '{print $2}' | sed 's/bw=//g' | "$TOOLS_PATH"/converter/to_MiB_s)
                    fi
                elif [[ "${file_system}" == "SplitFS-FIO" ]]; then
                    bash "$TOOLS_PATH"/setup.sh "$file_system" "null" "0"
                    export LD_LIBRARY_PATH="$BOOST_DIR"
                    ...
                    bash "$TOOLS_PATH"/setup.sh "$file_system" "main" "0"
                    if (( job == 1 )); then
                        BW=$(fio -filename="/mnt/pmem0/test" -fallocate=none -direct=0 -iodepth 1 -rw=read -ioengine=sync -bs="4K" -size="$fsize"M -name=test | grep READ: | awk '{print $2}' | sed 's/bw=//g' | "$TOOLS_PATH"/converter/to_MiB_s)
                    else
                        BW=$(fio -directory=/mnt/pmem0 -fallocate=none -direct=0 -iodepth 1 -thread -numjobs="$job" -rw=read -ioengine=sync -bs="4K" -size="$fsize"M -name=test | grep READ: | awk '{print $2}' | sed 's/bw=//g' | "$TOOLS_PATH"/converter/to_MiB_s)
                    fi
                ...
                if [[ "${file_system}" == "SLOTFS" ]]; then
                    ...
                    bash "$TOOLS_PATH"/setup.sh "$file_system" "main" "0"
                    if (( job == 1 )); then
                        BW=$(fio -filename="/mnt/pmem0/test" -fallocate=none -direct=0 -iodepth 1 -rw=write -ioengine=sync -bs="4K" -size="$fsize"M -name=test | grep WRITE: | awk '{print $2}' | sed 's/bw=//g' | "$TOOLS_PATH"/converter/to_MiB_s)
                    else
                        BW=$(fio -directory=/mnt/pmem0 -fallocate=none -direct=0 -iodepth 1 -thread -numjobs="$job" -rw=write -ioengine=sync -bs="4K" -size="$fsize"M -name=test | grep WRITE: | awk '{print $2}' | sed 's/bw=//g' | "$TOOLS_PATH"/converter/to_MiB_s)
                    fi     
                fi           
                ...
                if [[ "${file_system}" == "SLOTFS" ]]; then
                    ...
                    bash "$TOOLS_PATH"/setup.sh "$file_system" "main" "0"
                    if (( job == 1 )); then
                        BW=$(fio -filename="/mnt/pmem0/test" -fallocate=none -direct=0 -iodepth 1 -rw=randwrite -ioengine=sync -bs="4K" -size="$fsize"M -name=test | grep WRITE: | awk '{print $2}' | sed 's/bw=//g' | "$TOOLS_PATH"/converter/to_MiB_s)
                    else
                        BW=$(fio -directory=/mnt/pmem0 -fallocate=none -direct=0 -iodepth 1 -thread -numjobs="$job" -rw=randwrite -ioengine=sync -bs="4K" -size="$fsize"M -name=test | grep WRITE: | awk '{print $2}' | sed 's/bw=//g' | "$TOOLS_PATH"/converter/to_MiB_s)
                    fi     
                fi
