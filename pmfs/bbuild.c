				...
				u32 height, u32 btype)
{
	...
	/* only leaf node might be 4K, 2M, or 1G */
	if (height == 0) {
	...
	unsigned long *bitmap, unsigned long bsize, unsigned long scale)
{
	...
	while (1) {
		...
		ret = pmfs_alloc_insert_blocknode_map(sb, low << scale ,
				(next << scale) - 1);
		if (ret) {
			printk("PMFS: Error could not insert 0x%lx-0x%lx, %d\n",
				low << scale, ((next << scale) - 1), ret);
		}
		if (next == bsize)
			break;
	}
...
int pmfs_setup_blocknode_map(struct super_block *sb)
{
	...
	bm.bitmap_4k = kvzalloc(bm.bitmap_4k_size, GFP_KERNEL);
	bm.bitmap_2M = kvzalloc(bm.bitmap_2M_size, GFP_KERNEL);
	bm.bitmap_1G = kvzalloc(bm.bitmap_1G_size, GFP_KERNEL);

	if (!bm.bitmap_4k || !bm.bitmap_2M || !bm.bitmap_1G) {
		pmfs_dbg("PMFS: Fail to alloc bitmap\n");
		goto skip;
	}
