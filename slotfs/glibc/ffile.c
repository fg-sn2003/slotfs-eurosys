...
#define __open slotfs_open
#define __close slotfs_close
#define __read slotfs_read
#define __write slotfs_write
#define __lseek slotfs_lseek
...
FILE * _fopen ( const char * filename, const char * mode ) {
    ...
    /*_IO_file_open*/
    int fdesc = __open (AT_FDCWD, filename, omode|oflags, oprot);
    if (fdesc < 0)
        return NULL;

    fp->_fileno = fdesc;
    fp->_flags = _IO_MAGIC_SLOT;
