...
SLOTFS_DIR = os.path.abspath(os.path.join(BENCHMARK_DIR, "../slotfs/build"))
...
class Runner(object):
    ...
    def mount_killer(self, media, fs, mnt_path):
        (rc, dev_path) = self.init_media(media)
        if not rc:
            return False
        p = self.exec_cmd(
    ...
    def mount_splitfs(self, media, fs, mnt_path):
        ...
        #     return False
        p = self.exec_cmd(
            ' '.join([BENCHMARK_DIR + "/setup.sh SplitFS-FIO main"]), self.dev_null)
        if p.returncode != 0:
            return False
        p = self.exec_cmd("sudo chmod 777 " + mnt_path,
                          self.dev_null)
    ...
    def mount_madfs(self, media, fs, mnt_path):
        p = self.exec_cmd(
            ' '.join([BENCHMARK_DIR + "/setup.sh MadFS main"]), self.dev_null)
        if p.returncode != 0:
            return False
        p = self.exec_cmd("sudo chmod 777 " + mnt_path,
                          self.dev_null)
    ...
    def mount_slotfs(self, media, fs, mnt_path):
        p = self.exec_cmd(
            ' '.join([BENCHMARK_DIR + "/setup.sh SLOTFS null"]), self.dev_null)
        if p.returncode != 0:
            return False
        p = self.exec_cmd("sudo chmod 777 " + mnt_path,
                          self.dev_null)
    ...
    def mount_pmfs(self, media, fs, mnt_path):
        (rc, dev_path) = self.init_media(media)
        cmd = ' '.join([BENCHMARK_DIR + "/setup.sh PMFS main"])
        p = self.exec_cmd(cmd, self.dev_null)
        if p.returncode != 0:
            return False
        p = self.exec_cmd("sudo chmod 777 " + mnt_path)
    ...
    def fxmark_run_env(self, fs, bench, type, ncore, nsocket, node):
        ...
        if fs == "sufs":
            ...
            if bench == "fxmark":
                    fs_env = ("LD_PRELOAD=%s" % 
                              (os.path.normpath(os.path.join(MADFS_DIR, madfs.lib))))
        elif fs == "slotfs":
            if bench == "fxmark":
                    fs_env = ("LD_PRELOAD=%s " % (os.path.normpath(os.path.join(SLOTFS_DIR, slotfs.lib)))) + ("SLOTFS_MKFS=1")
        env = env + ' ' + fs_env
        return env
    ...
    def fxmark(self, media, fs, bench, ncore, nsocket, nfg, nbg, dio):
        ...
        if bin is self.fxmark_path:
            cmd = ' '.join([self.fxmark_run_env(fs, "fxmark", type, ncore, nsocket,
                            ...
                            "--filesys", "local" if fs == "splitfs" else fs,
                            "--pid", str(os.getpid()),
        ...
        if self.redirect:
            for l in p.stdout.readlines():
                ...
                if bin is self.fxmark_path and (fs == "strata" or fs == "splitfs" or fs == "slotfs" or fs == "madfs"):
        ...
        try:
            ...
            for (cnt, (media, fs, bench, ncore, nsocket, dio)) in enumerate(self.gen_config()):
                ...
                if not self.mount(media, fs, self.test_root):
                    self.log("# Fail to mount %s on %s. (%s)" % (fs, media, self.test_root))
                    continue
